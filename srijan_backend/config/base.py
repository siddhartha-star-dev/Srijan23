...
class Common(Configuration):
    ...
    INSTALLED_APPS = [
        ...
        # our apps
        "srijan_backend.apps.common.apps.CommonConfig",
        "srijan_backend.apps.accounts.apps.AccountsConfig",
        "srijan_backend.apps.main.apps.MainConfig",
    ]
    ...
    ROOT_URLCONF = "srijan_backend.urls"
    ...
    LOGGING = {
        ...
        "formatters": {
            ...
            "sql": {
                "()": "srijan_backend.loggers.SQLFormatter",
    ...
    SECURE_BROWSER_XSS_FILTER = env.bool(
        "SRIJAN_BACKEND_SECURE_BROWSER_XSS_FILTER",
        default=True,
    )
    SECURE_CONTENT_TYPE_NOSNIFF = env.bool(
        "SRIJAN_BACKEND_SECURE_CONTENT_TYPE_NOSNIFF",
        default=True,
    )
    SESSION_COOKIE_HTTPONLY = env.bool(
        "SRIJAN_BACKEND_SESSION_COOKIE_HTTPONLY",
        default=True,
    )
    SESSION_COOKIE_SECURE = env.bool(
        "SRIJAN_BACKEND_SESSION_COOKIE_SECURE",
        default=True,
    )
    CSRF_COOKIE_SECURE = env.bool(
        "SRIJAN_BACKEND_CSRF_COOKIE_SECURE",
        default=True,
    )
    X_FRAME_OPTIONS = env.str(
        "SRIJAN_BACKEND_X_FRAME_OPTIONS",
        default="SAMEORIGIN",
    )
    SECURE_HSTS_SECONDS = env.int(
        "SRIJAN_BACKEND_SECURE_HSTS_SECONDS",
        default=31536000,
    )  # 1 year
    ...
    ANNON_RATE = env.int(
        "SRIJAN_BACKEND_ANNON_THROTTLE_RATE_PER_MIUTE",
        default=50,
    )
    USER_RATE = env.int(
        "SRIJAN_BACKEND_USER_THROTTLE_RATE_PER_MIUTE",
        default=100,
    )
    ...
    REST_FRAMEWORK = {
        "DEFAULT_AUTHENTICATION_CLASSES": (
            "srijan_backend.apps.accounts.custom_auth.BearerAuthentication",
        ),
        ...
        "DEFAULT_PAGINATION_CLASS": (
            "srijan_backend.apps.common.pagination.DynamicPageSizePagination"
        ),
        "PAGE_SIZE": 20,
    ...
    REST_AUTH_SERIALIZERS = {
        ...
        "LOGIN_SERIALIZER": (
            "srijan_backend.apps.accounts.serializers.CustomLoginSerializer"
        ),
        "TOKEN_SERIALIZER": (
            "srijan_backend.apps.accounts.serializers.CustomTokenSerializer"
        ),
        "PASSWORD_RESET_SERIALIZER": (
            "srijan_backend.apps.accounts.serializers"
    ...
    PASSWORD_RESET_URL = env.str(
        "SRIJAN_BACKEND_PASSWORD_RESET_URL",
        "http://localhost:3000/",
    )
    ...
    LOGOUT_ON_PASSWORD_CHANGE = env.bool(
        "SRIJAN_BACKEND_LOGOUT_ON_PASSWORD_CHANGE",
        default=True,
    )
    ...
    OLD_PASSWORD_FIELD_ENABLED = env.bool(
        "SRIJAN_BACKEND_OLD_PASSWORD_ENABLED",
        default=True,
    )
    ...
    ACCOUNT_EMAIL_VERIFICATION = env.str(
        "SRIJAN_BACKEND_EMAIL_VERIFICATION",
        default="none",
    )
    ...
    COMMUNICATOR_NAME = env.str(
        "SRIJAN_BACKEND_MAIL_COMMUNICATOR_NAME",
        "Admin",
    )

    # For the docs
    CONTACT_EMAIL = env.str(
        "SRIJAN_BACKEND_CONTACT_EMAIL",
        "test@test.com",
    )
    SAMPLE_AUTH_TOKEN = env.str(
        "SRIJAN_BACKEND_SAMPLE_AUTH_TOKEN",
        "sjdhskjh3454343",
    )
    HOSTED_DOMAIN = env.str(
        "SRIJAN_BACKEND_HOSTED_DOMAIN",
        "https://ohuru.tech/",
    )
